...
import { X, Save, Clock, History, Loader2, FileText, Plus, CheckCircle2, AlertCircle, LayoutTemplate, Search, ShieldCheck } from 'lucide-react';
...
export function MedicalRecordModal({ isOpen, onClose, patientId, patientName, appointmentId, isInline = false }: Props) {
  ...
  const handleSign = async () => {
    ...
    const msg = 'Deseja assinar este prontuário via Lacuna (BirdID, VidaaS, etc.)? Após a assinatura digital, o documento não poderá mais ser editado.';
        ...
        <div className="flex gap-4">
          <button 
            ...
            className={`flex items-center gap-2 border-b-2 py-3 px-1 text-sm font-bold transition-colors ${
              view === 'new' ? 'border-sky-600 text-sky-600' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 border-b-2 py-3 px-1 text-sm font-bold transition-colors ${
              view === 'history' ? 'border-sky-600 text-sky-600' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <div className="space-y-4">
            {currentRecord?.status === 'SIGNED' ? (
              <div className="space-y-4">
                <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-emerald-900">Este prontuário já foi assinado</p>
                    <p className="text-xs text-emerald-700 mt-1">
                      Registros assinados não podem ser editados para garantir a integridade legal do documento.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="rounded-xl border border-sky-100 bg-sky-50/50 p-4 flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-1">
                      <div className="space-y-1">
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Data da Assinatura</p>
                        <p className="text-xs text-slate-700 font-medium">
                          {format(parseISO(currentRecord.signatureDate!), "dd/MM/yyyy 'às' HH:mm:ss")}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Integridade do Conteúdo (Hash)</p>
            ...
            <div className="bg-white">
              <div className="flex items-center justify-between mb-3">
                ...
                {currentRecord?.status !== 'SIGNED' && (
                  <button
                    onClick={() => setContent(prev => prev + '<p><strong>S (Subjetivo):</strong></p><p><strong>O (Objetivo):</strong></p><p><strong>A (Avaliação):</strong></p><p><strong>P (Plano):</strong></p>')}
              ...
              <div className="mt-8">
                ...
                <div className="space-y-3">
                  {currentRecord.audits.map((audit) => (
                    <div key={audit.id} className="rounded-lg border border-slate-100 bg-white p-3 text-xs">
                      <div className="flex items-center justify-between mb-2 text-slate-500">
                        <span className="font-bold">Alterado em: {format(parseISO(audit.changedAt), "dd/MM/yyyy 'às' HH:mm")}</span>
            ...
            {records.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-200 p-12 text-center">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {search ? 'Tente outros termos ou limpe a busca.' : 'Este paciente ainda não possui histórico de prontuários.'}
                </p>
              </div>
            ) : (
              records.map((record) => (
                <div key={record.id} className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2 text-xs font-bold text-sky-700 bg-sky-50 px-2 py-1 rounded-md">
                        <Clock className="h-3 w-3" />
                        {format(parseISO(record.createdAt), "dd 'de' MMMM 'às' HH:mm", { locale: ptBR })}
                      </div>
                        {record.status === 'SIGNED' ? (
                          <div className="flex flex-col gap-1">
                            ...
                            {record.signedHash && (
                              <p className="text-[8px] text-slate-400 font-mono line-clamp-1 ml-1" title={record.signedHash}>
                  ...
                  <div className="prose prose-sm prose-slate max-w-none text-slate-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: record.content }} />
                </div>
              ))
            )}
          </div>
